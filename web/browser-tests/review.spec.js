...
test('crossed order can be corrected and reversed without changing point coordinates',async({page})=>{
  ...
  await page.locator('#outline-warnings').getByRole('button',{name:'Check 3',exact:true}).first().click();
  ...
  await page.locator('#outline-direction').selectOption('clockwise');await expect(page.locator('#outline-warnings')).toContainText('counterclockwise');
...
test('clicking A opens baseline review and reference reminders persist',async({page})=>{
  ...
  await page.locator('#review-details summary').click();await expect(page.locator('#review-note')).toHaveValue('Check A–B tape sag');
...
test('changing units during a remeasurement requires refreshing its fields',async({page})=>{
  ...
  await page.locator('#unit').selectOption('feet');await expect(page.locator('#remeasure-status')).toContainText('changed while you were typing');await expect(page.locator('#remeasure-save')).toBeDisabled();
